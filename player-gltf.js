...
const $source = document.querySelector("#source");
{
	...
	$source.onchange = function() {
		...
		if(this.value.startsWith("embed:")) {
			...
			return captureEmbed().then(() => this.value = value, e => alert(e));
		}
		setStream(null);
		$embed.src = "";
		$embed.hidden = true;
	...
	async function captureEmbed() {
		if($embed.hidden || !sourceStream.active) {
			...
			const constraints = {audio: false, video: {
				cursor: "false", displaySurface: {ideal: "browser"},
				frameRate: {ideal: 60}, height: {max: 480}, // restrict for performance
			}};
			try {
		...
		$overlay.onpointermove = function(event) {
			...
			if(box.left <= event.clientX && event.clientX <= box.right
	...
	function resolveURL(url) {
		...
		if(m = url.match(/(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=))([A-Za-z0-9_\-]+)/))
			return {name: `youtube:${m[1]}`, embed_url: `https://www.youtube.com/embed/${m[1]}`};
		if(m = url.match(/twitch\.tv\/([A-Za-z0-9_\-]+)/))
			return {name: `twitch:${m[1]}`, embed_url: `https://player.twitch.tv/?channel=${m[1]}&autoplay=false&parent=${location.hostname}`};
		if(m = url.match(/drive\.google\.com\/file\/d\/([A-Za-z0-9_\-]+)/))
			return {name: `googledrive:${m[1]}`, embed_url: embedURL(`https://drive.google.com/uc?id=${m[1]}`)};
		...
		if(this.checked)
			for(const avatar of Object.values(avatars)) {
				...
				avatar.animRecorder = new AnimRecorder(() => {
					...
					avatar.humanPose.SetHipsPositionRotation(...avatar.motionDecoder.motions[0]);
					return {muscles: avatar.humanPose.muscles,
						rootT: avatar.humanPose.bodyPosition, rootQ: avatar.humanPose.bodyRotation};
				});
			}
		else 
			for(const avatar of Object.values(avatars)) {
...
let updateAvatars;
const $avatars = Array.from({length:2}, (_,i) => document.querySelector(`#avatar${i}`));
{
		...
		$avatar.onchange = function() {
			if(this.value === "file:") {
				...
				return openFiles($avatarFile, files => {
					for(const file of files)
						this.value = addAvatarOption(file);
					this.onchange();
				});
			}
			updateAvatars();
	...
	function addAvatarOption(file) {
		...
		for(const $select of $avatars)
			addOption($select, url, file.name);
		return url;
	}
}
// camera
...
function main() {
...

	{
		...
		updateAvatars = function() {
			for(const [i, $avatar] of $avatars.entries()) {
				...
				if((avatar ? avatar.src : "") === $avatar.value)
					continue;
	...
	function drawFrame(timeStamp) {
		...
		if($video.readyState >= $video.HAVE_CURRENT_DATA && $video.videoWidth) {
			gl.bindTexture(gl.TEXTURE_2D, wGLTF.loadTexture(gl, glob.textures.motion));
		...
		mat4.perspective(glob.uniforms.matrixVP, camera.yfov, aspect, camera.znear, camera.zfar);
		controls.multiplyMatrixV(glob.uniforms.matrixVP);
		mat4.translate(glob.uniforms.matrixVP,  glob.uniforms.matrixVP, camera.targetPos); 
		...
		// avatars
		const draws = Object.values(avatars).flatMap(avatar => avatar.draws||[])
		...
		if($record.checked)
			for(const avatar of Object.values(avatars))
				if(avatar.animRecorder.currentTime < 90000) {
					avatar.animRecorder.TakeSnapshot(glob.uniforms.deltaTime);
					$recordTime.textContent = `${avatar.animRecorder.currentTime.toFixed(0)}s`;
				}
		requestAnimationFrame(drawFrame);
	}
}
async function loadModel(gl, modelURL, glob) {
	...
	const mesh = vrm ? CreateMeshPlayer(gltf) : gltf.meshes[0];
	...
	const draws = mesh.primitives.map((prim,i) => {
		const {
			...
			pbrMetallicRoughness: { baseColorTexture: mainTexInfo, baseColorFactor: mainColor=[1,1,1,1] },
		...
		const tech = glob.techniques[alphaMode === "MASK" || depthWrite ? "MeshPlayerAlphaTest" : "MeshPlayer"];
		return {
			renderOrder: renderOrder, mirror: !vrm,
			primitive: prim,
			technique: tech,
			material: {
